...
const money = (n) => (typeof n === "number" && !Number.isNaN(n) ? `₹${n.toFixed(2)}` : "₹0.00")
...
const getOrigin = () => (typeof window !== "undefined" && window.location && window.location.origin) || ""
...
export default function Users() {
	...
	const filtered = useMemo(() => {
		if (!q && Object.values(filters).filter(Boolean).length === 0) return list
		const s = q.trim().toLowerCase()
		return list.filter((u) => {
			if (filters?.minBalance && (u.wallet_balance || 0) < +filters?.minBalance) return false
			if (filters?.maxBalance && (u.wallet_balance || 0) > +filters?.maxBalance) return false
	...
	const customerLink = async (uuid) => {
		try {
			const linkToShare = `${window.location.origin}/customer/${uuid}`
			try {
			...
			try {
				await navigator.share({ title: "Your Sehat Box link", url: linkToShare })
			} catch {
				alert("Customer link copied!")
			}
		} catch (err) {
			console.error("copyMagicLink error", err)
			alert(err?.response?.data?.error || "Failed to generate magic link. Check if you're logged in as admin.")
		}
	}
	...
	return (
		<div className='space-y-4'>
			<div className='flex items-center justify-between'>
				...
				<button onClick={onNew} className='px-3 py-2 rounded-md bg-black text-white'>
					+ New User
				</button>
			</div>
			...
			<div className='flex gap-2'>
				...
				<button onClick={fetchUsers} className='px-3 py-2 rounded-md border bg-white'>
					Search
				</button>
			</div>
					...
					<tbody>
						{loading ? (
							...
							filtered.map((u, i) => {
								...
								const createdText = dateTimeString(u.created_at)
								const wallet =
			...
			{modalOpen && <UserModal initial={editing} onClose={() => setModalOpen(false)} onSaved={onSaved} />}

			{balModalOpen && balanceUser && (
				<AddBalanceModal user={balanceUser} onClose={() => setBalModalOpen(false)} onSaved={onBalanceSaved} />
			)}
...
function AddBalanceModal({ user, onClose, onSaved }) {
	...
	const submit = async (e) => {
		...
		try {
			...
			setErr(
				e?.response?.data?.message || e?.response?.data?.error || e?.message || "Failed to add wallet balance"
			)
		} finally {
			setSaving(false)
		}
...
function normalizeAddresses(addrs) {
	...
	const out = arr
		.map((a) => {
			const isDef = a?.is_default === "1" || a?.is_default === 1 || a?.is_default === true
			const item = {
...
function UserModal({ initial, onClose, onSaved }) {
	...
	const [type, setType] = useState(Number.isFinite(Number(initial?.type)) ? Number(initial.type) : 1)
	const [status, setStatus] = useState(Number.isFinite(Number(initial?.status)) ? Number(initial.status) : 1)
	...
	const setDefault = (idx) => {
		setAddresses((prev) => prev.map((a, i) => ({ ...a, is_default: i === idx ? "1" : "0" })))
	}
	...
	const onSubmit = async (e) => {
		...
		try {
			...
			setErr(e?.response?.data?.message || e?.response?.data?.error || e?.message || "Failed to save user")
		} finally {
			setSaving(false)
		}
	}
			...
			<div className='w-full max-w-2xl bg-white rounded-xl shadow max-h-[85vh] overflow-y-auto'>
				<div className='p-4'>
					<div className='flex items-center justify-between'>
						...
						<button className='px-2 py-1 rounded border' onClick={onClose} disabled={saving} title='Close'>
							✕
						</button>
					</div>
					...
					<form onSubmit={onSubmit} className='mt-3 space-y-3'>
						...
						{Number(type) === 1 && (
							<div className='space-y-2'>
								<div className='flex items-center justify-between'>
									<label className='text-sm font-medium'>Addresses ({addresses.length})</label>
									<button type='button' onClick={addAddress} className='px-2 py-1 rounded border'>
										+ Add address
									</button>
								</div>
										...
										<div key={idx} className='border rounded-md p-3 space-y-2'>
											...
											<div className='space-y-1'>
												...
												<input
													...
													onChange={(e) => updateAddr(idx, "location_url", e.target.value)}
												/>
											</div>
