...
export default function MealPlan() {
	...
	const visibleDishes = useMemo(() => {
		...
		const slot = mealSlots.find((s) => String(s.meal_id) === String(selectedMealId))
		if (!slot) return []
		const arr = []
		slot.dish_ids.forEach((dishId) => {
			const dObj = allDishesMap[dishId] || allDishesMap[String(dishId)]
	...
	useEffect(() => {
		...
		;(async () => {
			try {
				...
				// load all meal plans
				const mpRes = await api.get("/meal-plan")
				...
				// pick active plan
				const activePlan = plansArray.find((p) => Number(p.status) === 1) || null
				...
				const initialSlots = Array.isArray(activePlan.plan)
					? activePlan.plan.map((slot) => ({
							...
							dish_ids: Array.isArray(slot.dish_id) ? slot.dish_id : []
					  }))
					: []

				const slotsWithDishes = initialSlots.filter((s) => s.dish_ids.length > 0)
				...
				for (const slot of slotsWithDishes) {
					for (const dishId of slot.dish_ids) {
						try {
							const dRes = await api.get(`/dishes/${dishId}`)
				...
				if (!alive) return
				setErr(e?.response?.data?.message || e?.response?.data?.error || "Unable to load meal plan.")
				setMealPlan(null)
				setMealSlots([])
				setSelectedMealId("")
				setAllDishesMap({})
	...
	const checkExistingOrderForSelection = async (chosenMealId, planOverride) => {
		...
		try {
			...
			if (arr.length > 0) {
				const found = arr[0]
				const isCancelled = Number(found.status) === CANCELLED_STATUS_CODE
	...
	const submitCartOrder = async () => {
		...
		try {
			...
			const mergedInstructions = cartItems
				.map((line) => {
					const dishName = line.dish.title || line.dish.name || "Dish"
			...
			if (existingOrder && !cartLocked) {
				await api.put(`/orders/${existingOrder._id}`, body)

				alert("✅ Order updated!")

				const refreshed = await api.get(`/orders/${existingOrder._id}`)
	...
	const MacroChip = ({ value, label }) => (
		<div className='flex flex-col items-center justify-center border border-gray-300 rounded-xl px-3 py-2 min-w-[70px] bg-white'>
			<div className='text-[16px] font-semibold text-gray-900 leading-none'>{value}</div>
			<div className='text-[13px] leading-none text-gray-600 mt-1'>{label}</div>
		</div>
	)
		...
		return (
			<div className='border-b border-gray-300 py-5'>
				...
				<div className='mt-4 flex flex-wrap gap-2 text-[14px] text-gray-900'>
					...
					<MacroChip value={`${fmtNumber(macros.protein)}g`} label='protein' />
					<MacroChip value={`${fmtNumber(macros.carbs)}g`} label='carbs' />
	...
	if (err || !mealPlan || !mealSlots.length) {
		if (err === "No active meal plan." || !mealPlan || !mealSlots.length) {
			return (
		...
		>
			...
			>
				<div className='flex items-center justify-between mb-3'>
					...
					<button className='text-sm text-gray-500' onClick={() => setShowCartDrawer(false)}>
						Close
					</button>
				</div>
				...
				<div className='overflow-y-auto flex-1 pr-1'>
					{cartItems.length === 0 ? (
						...
						cartItems.map((line) => {
							...
							return (
								<div key={dishId} className='border-b border-gray-200 pb-4 mb-4'>
										...
										<div className='flex-shrink-0'>
											...
											>
												<button
													className='text-xl leading-none px-1 disabled:opacity-50'
													disabled={disabled}
													onClick={() => !disabled && setDishQty(dish, line.qty - 1)}
												>
													–
												</button>
									...
									<div className='mt-3'>
										...
										<textarea
											...
											onChange={(e) => updateInstructions(dishId, e.target.value)}
											disabled={disabled}
										/>
									</div>
								</div>
				...
				{cartItems.length > 0 && (
					<div className='mt-2 text-[13px] text-gray-700 leading-snug border-t border-gray-300 pt-3'>
						<div className='text-gray-900 mb-1 font-medium'>Today's Intake (Selected Items)</div>
						<div className='flex flex-wrap gap-3 text-[13px]'>
							<div>
								<span className='font-medium'>{fmtNumber(cartNutritionTotals.calories)}</span> kcal
							</div>
							<div>
								<span className='font-medium'>{fmtNumber(cartNutritionTotals.protein)}</span> g protein
							</div>
							<div>
								<span className='font-medium'>{fmtNumber(cartNutritionTotals.carbs)}</span> g carbs
							</div>
							<div>
								<span className='font-medium'>{fmtNumber(cartNutritionTotals.fat)}</span> g fat
							</div>
						</div>

						<div className='mt-2 text-[13px] text-gray-600'>
							Cart Total:{" "}
							<span className='font-semibold text-gray-900'>₹{fmtNumber(cartTotalAmount)}</span>
						</div>

						{submitError && <div className='text-red-600 text-[13px] mt-2'>{submitError}</div>}
						...
						>
							{submitting ? "Submitting..." : `Submit • ₹${fmtNumber(cartTotalAmount)}`}
						</button>
					</div>
				)}
		...
		initialMealChoiceOpen && mealSlots.length > 1 ? (
			<div className='fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4'>
				<div className='bg-white w-full max-w-sm rounded-xl p-5 shadow-2xl'>
					<div className='text-base font-semibold text-gray-900 mb-2'>Choose a meal</div>
					<div className='text-sm text-gray-600 mb-4'>Which meal do you want to order right now?</div>
						...
						{mealSlots.map((slot) => (
							...
							>
								<div className='flex flex-col'>
									<div>{MEAL_NAME_MAP[slot.meal_id] || slot.meal_id}</div>
								...
								{mealSlots.map((slot) => (
									...
									>
										<div className='font-medium text-green-800'>
											{MEAL_NAME_MAP[slot.meal_id] || slot.meal_id}
										</div>
										{MEAL_TIME_MAP[slot.meal_id] ? (
						...
						<div className='flex flex-col gap-3'>
							<div className='flex flex-col sm:flex-row gap-3'>
								...
								>
									...
									<span className='underline font-medium'>View Cart &gt;</span>
								</button>
							</div>
						</div>
					)}
