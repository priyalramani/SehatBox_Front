...
function SearchableSelect({ value, onChange, options, placeholder = "-- Choose --", disabled = false }) {
  ...
  function onKeyDown(e) {
    ...
    if (e.key === "ArrowDown") {
      ...
      listRef.current?.children?.[Math.min(highlight + 1, filtered.length - 1)]?.scrollIntoView({ block: "nearest" });
  ...
  return (
    <div ref={rootRef} className="relative" onKeyDown={onKeyDown}>
      <button
        type="button"
        disabled={disabled}
        className={`w/full border rounded-md px-3 py-2 text-left bg-white ${disabled ? "opacity-60 cursor-not-allowed" : "cursor-default"}`}
        onClick={() => {
          if (disabled) return;
          setOpen((o) => !o);
          setTimeout(() => inputRef.current?.focus(), 0);
      ...
      >
        {selected ? <span className="text-gray-900">{selected.label}</span> : <span className="text-gray-500">{placeholder}</span>}
      </button>
...
export default function AddOrder() {
  ...
  const userOptions = useMemo(() => {
    return users.map((u) => {
      ...
      const label = hasName
        ? (u.mobile ? `${u.name} — +91 ${u.mobile}${walletPart}` : `${u.name}${walletPart}`)
        : (u.mobile ? `+91 ${u.mobile}${walletPart}` : `Unnamed${walletPart}`);
      const search = `${(u.name || "").toLowerCase()} ${(u.mobile || "").toLowerCase()}`;
      return { value: u.id, label, search };
    });
  }, [users]);
  ...
  const activeDishes = useMemo(
    () => dishes.filter((d) => Number(d.status) === 1 || d.status === true),
    [dishes]
  );
  ...
  const lines = useMemo(
    () =>
      activeDishes
        .map((d) => ({ ...d, quantity: toNum(qty[d.id] ?? 0) || 0 }))
  ...
  const submitOrder = async () => {
    ...
    if (error) { setDateErr(error); return; }

    const dish_details = lines.map((l) => ({ dish_uuid: l.id, quantity: l.quantity }));
    const common = { dish_details, meal_id: selectedMealId, instructions: String(instructions || "").trim(), for_date: iso };
    ...
    try {
      const calls = selectedUserIds.map((uid) =>
        api.post("/orders", { ...common, user_uuid: uid })
          ...
          .catch((err) => ({
            ok: false,
            uid,
            errMsg: err?.response?.data?.message || err?.response?.data?.error || err?.message || "Unknown error",
          }))
      );
      ...
      if (bad.length > 0) {
        ...
        if (insuff.length) {
          summary += `\n${insuff.length} order${insuff.length === 1 ? "" : "s"} unsuccessful due to insufficient balance:`;
      ...
      const m = e?.response?.data?.message || e?.response?.data?.error || e?.message || "Failed to place order(s).";
      setMsg(m); alert(m);
    } finally {
      setSubmitting(false);
    }
  ...
  const saveEdit = async () => {
    ...
    const dish_details = lines.map((l) => ({ dish_uuid: l.id, quantity: l.quantity }));
    const payload = { dish_details, meal_id: selectedMealId, instructions: String(instructions || "").trim(), for_date: iso };
    ...
    <section className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        {isEditingExisting && (
          <div className="flex gap-2">
            {viewMode ? (
              <>
                <button className="px-3 py-2 rounded-md border" onClick={() => navigate(-1)}>Back</button>
                <button className="px-3 py-2 rounded-md bg-blue-600 text-white" onClick={() => setViewMode(false)}>Edit</button>
              </>
            ) : (
              <>
                <button className="px-3 py-2 rounded-md border" onClick={() => setViewMode(true)} disabled={submitting}>Cancel</button>
                <button className="px-3 py-2 rounded-md bg-green-600 text-white" onClick={saveEdit} disabled={submitting}>
                ...
                <div key={d.id} className="border rounded-md p-3 flex flex-col gap-2">
                  ...
                  <div className="mt-1 flex items-center gap-2">
                    ...
                    <input
                      ...
                      value={q}
                      onChange={(e) => !viewMode && setQty((prev) => ({ ...prev, [d.id]: Math.max(0, toNum(e.target.value)) }))}
                      disabled={isEditingExisting && viewMode}
                    />
                  </div>
                </div>
      ...
      <div className="p-4 border rounded-md flex items-center justify-between">
        <div className="text-sm">
          <div className="text-gray-600">Items: <b>{lines.reduce((s, l) => s + l.quantity, 0)}</b></div>
          <div className="text-base">Total: <b>{money(total)}</b></div>
        </div>

        {isEditingExisting ? (
          viewMode ? (
            <button className="px-4 py-2 rounded-md bg-blue-600 text-white" onClick={() => setViewMode(false)}>
              Edit
            </button>
          ) : (
            <button
          ...
          >
            {submitting ? "Submitting..." : `Submit ${selectedUserIds.length || ""} Order${selectedUserIds.length > 1 ? "s" : ""}`}
          </button>
        )}
      </div>
